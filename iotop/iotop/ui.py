...
class IOTopUI(object):
    ...
    def get_data(self):
        def format(p):
            ...
            line = (pid_format + '  %4s %-8s %12s  %12s %s ') % (
    ...
    def refresh_display(self, first_time, total, actual, duration):
        ...
        ##if lines: 
        _current_time = time.strftime('%d/%m/%Y %H:%M:%S  ')
        global firstTime
        if firstTime:
            firstTime=False;
            ...
            for i in _lines:
                file.write(i+"\n")
            file.write("\n\n")'''
        ##means that tracingpid is not active    
                
        if not lines and self.options.pids:
            pid_format = '%%%dd' % MAX_PID_WIDTH
            tracingLine = (pid_format + '  %40s') % (
                self.options.pids[0],
        ...
        if self.options.time:
            ...
            current_time = time.strftime('%d/%m/%Y %H:%M:%S  ',local_time)
...
def main():
    ...
    except locale.Error:##modify to nothing
        pass
    parser = optparse.OptionParser(usage=USAGE, version='iotop ' + VERSION)
    ##modify
    parser.add_option('-f', '--file', type='string', dest='fpath',

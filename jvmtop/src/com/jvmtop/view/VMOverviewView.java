...
public class VMOverviewView extends AbstractConsoleView
{
  ...
  public void setPid(int pid){processid=pid;}
  ...
  public void printView() throws Exception
  {
    ...
    {
    ///start
      if (JvmTop.tracingPid!=0)
      {
    	  if (vmInfo.getId()!=JvmTop.tracingPid)
    		  continue;
      }
      ...
      else if (vmInfo.getState() == VMInfoState.ATTACHED_UPDATE_ERROR)
      {
        ...
        fw.write(String.format("%5d %-15.15s [ERROR: Could not fetch telemetries (Process DEAD?)] %n",
                vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName())));
      }
      else if (vmInfo.getState() == VMInfoState.ERROR_DURING_ATTACH)
      {
    	  /*/System.out.printf("%5d %-15.15s [ERROR: Could not attach to VM] %n",
            vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName()));/*/
        fw.write(String.format("%5d %-15.15s [ERROR: Could not attach to VM] %n",
            vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName())));
      }
      else if (vmInfo.getState() == VMInfoState.CONNECTION_REFUSED)
      {
    	  /*/System.out.printf(
            ...
            vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName()));/*/
        fw.write(String.format("%5d %-15.15s [ERROR: Connection refused/access denied] %n",
            vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName())));
      }
  ...
  private void printVM(VMInfo vmInfo) throws Exception
  {
    ...
    /*/System.out
        .printf(
            ...
            deadlockState);/*/
    fw.write(String.format("%28s  %5d %-15.15s %5s %5s %5s %5s %5.2f%% %5.2f%% %-5.5s %8.8s %4d %2.2s%n",
    		new GregorianCalendar().getTime(), vmInfo.getId(), getEntryPointClass(vmInfo.getDisplayName()),
            toMB(vmInfo.getHeapUsed()), toMB(vmInfo.getHeapMax()),
            toMB(vmInfo.getNonHeapUsed()), toMB(vmInfo.getNonHeapMax()),
            vmInfo.getCpuLoad() * 100, vmInfo.getGcLoad() * 100,
            vmInfo.getVMVersion(), vmInfo.getOSUser(), vmInfo.getThreadCount(),
 ...
 * @throws IOException 
  ...
  */
  private void printHeader() throws IOException
  {
    ...
    ///start
	if (JvmTop.firstTime){
		fw.write(String.format("%-28s  %5s %-15.15s %5s %5s %5s %5s %6s %6s %5s %8s %4s %2s%n",
    	"LOCAL-TIME", "PID", "MAIN-CLASS", "HPCUR", "HPMAX", "NHCUR", "NHMAX", "CPU", "GC",
        "VM", "USERNAME", "#T", "DL"));
		JvmTop.firstTime=false;
...
/* (non-Javadoc)
 * @see com.jvmtop.view.ConsoleView#printView()
 */
}
